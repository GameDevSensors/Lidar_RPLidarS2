...
namespace RPLidar
{
    ...
    public partial class Lidar
    {
        ...
        public bool StartScan(ScanMode mode)
        {
            switch (mode)
            {
                ...
                case ScanMode.ExpressExtended:
                    Debug.LogError("Express extended scan not yet supported.");
                    return false;

                default:
                    Debug.LogError("Invalid scan mode, could be a bug.");
                    return false;
            }
        }

        public bool SetMotorSpeed(ushort speed)
        {
            if (!SendCommand(Command.SetMotorRPM, BitConverter.GetBytes(speed), "motor speed ctrl"))
            {
                return false;
            }
        ...
        public List<Measurement> GetMeasurements()
        {
            ...
            if (usage > 50)
            {
                Debug.LogWarning($"Receive buffer is {usage}% full, should read measurements faster.");
            }
            ...
            {
                case null:
                    Debug.LogError("No scan mode active.");
                    return null;
                ...
                default:
                    Debug.LogError($"Invalid scan mode '{activeMode}', it could be a bug");
                    return null;
            }
        }
        ...
        private List<Measurement> GetLegacyMeasurements()
        {
            ...
            for (int i = 0; i < buffer.Length; i += 5)
            {
                ...
                {
                    Debug.LogError("Receieved invalid scan data (start flags not inverted).");
                    return null;
                }
                ...
                if ((buffer[i + 1] & 1) != 1)
                {
                    Debug.LogError("Receieved invalid scan data (check bit not set).");
                    return null;
                }
        ...
        private bool ParseExpressLegacyMeasurementsPacket(byte[] buffer, List<Measurement> measurements, out float startAngle)
        {
            ...
            {
                Debug.LogError("Received invalid scan packet (invalid sync).");
                return false;
            }
            ...
            {
                Debug.LogError("Received invalid scan packet (invalid checksum).");
                return false;
            }
