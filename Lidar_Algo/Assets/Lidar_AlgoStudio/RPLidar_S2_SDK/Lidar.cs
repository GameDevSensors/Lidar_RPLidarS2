...
namespace RPLidar
{
    ...
    public partial class Lidar
    {
        ...
        public Lidar(string portName = "", int baudRate = 1000000, int readBufferSize = 4096)
            : this()
        {
            PortName = portName;
            Baudrate = baudRate;
        ...
        public bool IsOpen
        {
            get
            {
                ...
                {
                    UnityEngine.Debug.LogException(ex/*, "Error at checking port status."*/);
                    return false;
                }
            }
        }
        ...
        public bool Open()
        {
            try
            {
                ...
                // Imprimimos el puerto que se ha abierto
                UnityEngine.Debug.Log($"Puerto abierto: {port.PortName}");
                // Buscamos el objeto de texto llamado error
                return port.IsOpen;
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError(ex/*, "Error at opening port."*/);
                // Buscamos el objeto de texto llamado error
                return true;
            }
        }
        ...
        public bool Close()
        {
            ...
            {
                UnityEngine.Debug.LogError(ex/*, "Error at closing port."*/);
                return false;
            }
        }
        ...
        private bool GetBytesToRead(out int count)
        {
            ...
            {
                UnityEngine.Debug.LogError(ex/*, "Error at checking readable bytes count."*/);
                count = 0;
                return false;
            }
        }
        ...
        private bool SendCommand(Command command, string commandName)
        {
            ...
            {
                UnityEngine.Debug.LogError(ex/*, $"Error at sending {commandName} command."*/);
                return false;
            }
        ...
        private bool SendCommand(Command command, byte[] payload, string commandName)
        {
            ...
            {
                UnityEngine.Debug.Log(commandName);
                UnityEngine.Debug.LogError(ex/*, $"Error at sending command {commandName}."*/);
                return false;
            }
        ...
        private Descriptor ReadDescriptor(string responseName)
        {
            ...
            while (true)
            {
                ...
                {
                    UnityEngine.Debug.LogError($"Timeout at receiving descriptor for {responseName}.");
                    return null;
                }
                catch (Exception ex)
                {
                    UnityEngine.Debug.LogError(ex/*, $"Error at receiving descriptor for {responseName}."*/);
                    return null;
                }
        ...
        private bool CheckDescriptor(Descriptor readDescriptor, Descriptor expectedDescriptor, string responseName)
        {
            ...
            {
                UnityEngine.Debug.LogError($"Expected {responseName} descriptor length {expectedDescriptor.Length}, got {readDescriptor.Length}.");
                return false;
            }

            if (expectedDescriptor.IsSingle != readDescriptor.IsSingle)
            {
                UnityEngine.Debug.LogError($"Expected {responseName} descriptor single to be {expectedDescriptor.IsSingle}, got {readDescriptor.IsSingle}.");
                return false;
            }

            if (expectedDescriptor.DataType != readDescriptor.DataType)
            {
                UnityEngine.Debug.LogError($"Expected {responseName} descriptor data type 0x{expectedDescriptor.DataType:X2}, got 0x{readDescriptor.DataType:X2}.");
                return false;
            }
        ...
        private byte[] ReadResponse(int length, string responseName)
        {
            ...
            while (dataIndex < length)
            {
                ...
                {
                    UnityEngine.Debug.LogError($"Timeout at receiving data for {responseName}.");
                    return Array.Empty<byte>();
                }
                catch (Exception ex)
                {
                    UnityEngine.Debug.LogError(ex/*, $"Error at receiving data for {responseName}."*/);
                    return Array.Empty<byte>();
                }
        ...
        private bool FlushInput()
        {
            ...
            {
                UnityEngine.Debug.LogError(ex/*, "Error on flushing input buffer."*/);
                return false;
            }
        }
        ...
        public bool Reset(int waitTime = 700)
        {
            ...
            if (length > 0)
            {
                ...
                UnityEngine.Debug.LogError($"Reset message: {msg}");
            }
        ...
        private byte[] GetConfigurationType(ConfigType configType, byte[] requestPayload, int? expectedResponseLength)
        {
            ...
            {
                UnityEngine.Debug.LogError($"Expected {responseName} response type 0x{configType:X2}, got 0x{responseType:X2}.");
                return Array.Empty<byte>();
            }
